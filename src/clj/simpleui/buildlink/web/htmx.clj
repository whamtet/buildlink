(ns simpleui.buildlink.web.htmx
  (:require
   [simpleui.buildlink.env :refer [dev?]]
   [simpleui.buildlink.i18n :refer [i18n]]
   [simpleui.buildlink.web.resource-cache :as resource-cache]
...
(defn page-htmx [{:keys [css js hyperscript?]} & body]
  (page
   [:head
    [:meta {:charset "UTF-8"}]
    [:title (i18n "Buildlink")]
    [:meta {:property "og:title" :content "Buildlink Platform"}]
    [:meta {:property "og:type" :content "website"}]
    [:meta {:property "og:url" :content "https://buildlink.simpleui.io/"}]
    [:meta {:property "og:image" :content "https://buildlink.simpleui.io/logo.jpg"}]
